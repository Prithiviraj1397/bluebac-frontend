...
//Images
import logo1 from "../../../assets/images/brand-logos/logo1.svg"
import logo2 from "../../../assets/images/brand-logos/toggle-logo.svg"
import logo3 from "../../../assets/images/brand-logos/logo2.svg"
import logo4 from "../../../assets/images/brand-logos/toggle-dark.svg"
import logo5 from "../../../assets/images/brand-logos/logo1.svg"
import logo6 from "../../../assets/images/brand-logos/toggle-logo.svg"
...
const Sidebar = ({ local_varaiable, ThemeChanger }: any) => {
  ...
  function Onhover() {
    ...
    if (
      (theme.basicReducer.toggled == "icon-overlay-close" || theme.basicReducer.toggled == "detached-close") &&
      theme.basicReducer.iconOverlay != "open"
    ) {
            ...
            if (items.children) {
              items.children.filter((submenu: any) => {
                ...
                if (dd === submenu.path + "/") {
                  const theme = store.getState()
                  items.active =
                    theme.basicReducer.dataNavLayout == "horizontal" || theme.basicReducer.dataNavStyle == "icon-hover" ? false : true
                  items.selected = true
                  submenu.active = true
        ...
        if (!(local_varaiable.dataVerticalStyle.dir === "rtl")) {
          ...
          if (Math.abs(check) > Math.abs(marginRightValue)) {
            ...
            menuNav.style.marginInlineEnd =
              Number(menuNav.style.marginInlineEnd.split("px")[0]) - Math.abs(mainContainer1Width) + "px"
  ...
  const MenuOpen = (_event: any) => {
    ...
    const theme = store.getState()
    if (localStorage.ynexverticalstyles === "doublemenu" && theme.basicReducer.toggled !== "double-menu-open") {
                ...
                {MENUITEMS.map(levelone => (
                  <Fragment key={Math.random()}>
                    ...
                    {levelone.Items.map((leveltwo: any) =>
                      leveltwo.type === "link" ? (
                        <li className={`slide ${leveltwo.active ? "active" : ""}`} key={Math.random()}>
                          <Link
                            ref={ulRef}
                            onClick={_event => {
                            ...
                            to={leveltwo.path + "/"}
                            className={`side-menu__item ${leveltwo.selected ? "active" : ""}`}
                          >
                            {leveltwo.icon}{" "}
                            <span className="side-menu__label">
                              {leveltwo.title}
                              {leveltwo.badgetxt ? (
                                <span className={leveltwo.class}>{leveltwo.badgetxt}</span>
                              ) : (
                                ""
                              )}
                            </span>
                        ...
                        <li
                          className={`slide has-sub ${leveltwo.active ? "open" : ""} ${
                        ...
                        >
                          ...
                          <ul
                            ref={ulElement}
                            className={`slide-menu child1 ${leveltwo.active ? "active" : ""} ${
                              leveltwo.active && localStorage.ynexverticalstyles == "doublemenu"
                                ? "double-menu-active"
                                : ""
                            }
              `}
                            style={leveltwo.active ? { display: "block" } : { display: "none" }}
                          >
                              ...
                              <Fragment key={Math.random()}>
                                {thirdlayer.type === "link" ? (
                                  <li className={`slide ${thirdlayer.active ? "active" : ""}`}>
                                    {thirdlayer.path == "/" ? (
                                      ...
                                      >
                                        {thirdlayer.icon}
                                        {thirdlayer.title}
                                        <span className={thirdlayer.class}>{thirdlayer.badgetxt}</span>
                                      </Link>
                                    )}
                                  </li>
                                ) : thirdlayer.type === "sub" ? (
                                  <li
                                    className={`slide has-sub ${thirdlayer.active ? "open" : ""} ${
                                  ...
                                  >
                                    ...
                                    <ul
                                      className={`slide-menu child2 ${thirdlayer.active ? "active" : ""}`}
                                      style={thirdlayer.active ? { display: "block" } : { display: "none" }}
                                    >
                                          ...
                                          {fourthlayer.type === "link" ? (
                                            <li className={`slide ${fourthlayer.active ? "active" : ""}`}>
                                              <Link
                                                ...
                                                to={fourthlayer.path == "/" ? "#" : fourthlayer.path + "/"}
                                            ...
                                            <li className={`slide has-sub ${fourthlayer.active ? "open" : ""}`}>
                                              <a
                                                href="#"
                                                className="side-menu__item"
                                                onClick={evnt => {
                                              ...
                                              >
                                                {fourthlayer.icon} <span className="">{fourthlayer.title}</span>
